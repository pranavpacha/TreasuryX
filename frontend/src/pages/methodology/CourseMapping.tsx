...
const CV_ROWS: Row[] = [
  ...
  { topic: "Segmentation — thresholding, morphological processing (Module 2)", status: "implemented", where: "Financial Image Intelligence + Segmentation", link: "/intelligence/financial-image", note: "GrabCut chart-region isolation feeds OCR in production; benchmark at /evidence/cv/segmentation" },
  ...
  { topic: "CNN review, CNNs for recognition (Module 3)", status: "implemented", where: "CNN vs. ViT (evidence)", link: "/evidence/cv/models", note: "Trained from scratch, real metrics; kept as an evaluation exercise since it's trained on synthetic data only — see Computer Vision Methodology" },
  ...
  { topic: "CNNs for segmentation — FCN, U-Net, Mask-RCNN (Module 3, Lab 5)", status: "partial", where: "Segmentation (production + evidence)", link: "/intelligence/financial-image", note: "Classical (GrabCut) segmentation implemented for the in-scope chart-isolation task; U-Net/medical-dataset training out of scope (wrong domain)" },
  ...
  { topic: "Attention models, Transformer review, Vision Transformers (Module 4, Lab 7)", status: "implemented", where: "CNN vs. ViT (evidence)", link: "/evidence/cv/models", note: "A ViT written from scratch (patch embedding, class token, multi-head self-attention), directly compared against the CNN" },
...
const CG_ROWS: Row[] = [
  ...
  { topic: "VR/AR/MR introduction, architecture, hardware, DOF, tracking (Module 4)", status: "partial", where: "VR/AR Concepts (evidence)", link: "/evidence/graphics/vr-ar", note: "Documented conceptually; no VR/AR hardware used, per the project's laptop-only requirement" },
  { topic: "Human factors in VR — FOV, latency, motion sickness, ergonomics (Module 4)", status: "partial", where: "VR/AR Concepts (evidence)", link: "/evidence/graphics/vr-ar" },
